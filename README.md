...
`make migrate` is used to apply migrations to a previously launched database, for example, by `make db-up` command.
...
Loads the three templates and two demo loans to populate db with data without need of manual operations.

The demo loans carry phone numbers with `999` country code `999`, so testing against a real MessageBird wont really send an SMS.

### Tests

```bash
make test               # unit tests
make test-integration   # integration tests, againsta a real database
make hooks              # lint before every push
git push --no-verify    # push skipping the hook
```

Tests that need PostgreSQL live in `test/integration` and controlled by `TEST_DATABASE_URL` with default set to `localhost:5432`.

### Running against MessageBird

Supply real `MESSAGEBIRD_BASE_URL` (`https://rest.messagebird.com`) and `MESSAGEBIRD_ACCESS_KEY`.

The live API has never been reached, because registering a MessageBird account did not succeeded (including free 10 trial SMS). The client is covered instead by tests against a local server that replays the response shapes MessageBird documents, including error replies.
...
A template body may use four placeholders, in single braces and `snake_case`: `{credit_number}`, `{full_name}`, `{amount}` and `{due_date}`. Three name a column and `{amount}` that renders the stored minor units and currency together as one readable figure (`amount_minor` column).
...
- **For at-most-once guarantee** - mark the row as attempted before calling the provider and never retry an ambiguous
  outcome.
- **For at-least-once guarantee** - retry until the provider confirms, and deduplicate with an idempotency key so a
